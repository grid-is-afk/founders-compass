  ...
  return (
    <form onSubmit={handleSubmit} className="border border-border/60 rounded-lg bg-muted/10 p-3 space-y-2.5">
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-foreground" htmlFor={`label-${capitalType}`}>
          Finding label
        </label>
        <Input
          id={`label-${capitalType}`}
          placeholder="e.g. Hire senior engineer Q3…"
          value={label}
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-foreground" htmlFor={`partner-${capitalType}`}>
          Partner <span className="text-muted-foreground font-normal">(optional)</span>
        </label>
        <Input
          id={`partner-${capitalType}`}
...
export function GrowQ2Panel({ clientId, nextPhase, onPhaseComplete }: GrowQ2PanelProps) {
  const { data: growItems = [], isLoading } = useClientGrow(clientId);
