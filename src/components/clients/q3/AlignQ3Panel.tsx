...
const ALIGN_CHECKLIST: AlignItem[] = [
  { key: "q3_align_protection_review", group: "Progress Review", label: "Review Protection Objectives" },
  { key: "q3_align_value_review", group: "Progress Review", label: "Review Value Enhancement goals" },
  { key: "q3_align_objectives", group: "Quarterly Roadmap", label: "Set Objectives and Milestones" },
  { key: "q3_align_champions", group: "Quarterly Roadmap", label: "Identify Champions" },
  { key: "q3_align_referrals", group: "Referrals", label: "Make referrals to partners" },
];
...
export function AlignQ3Panel({ clientId, nextPhase, onPhaseComplete }: AlignQ3PanelProps) {
  ...
  const handleCompleteChapter = async () => {
    try {
      await updateClient.mutateAsync({ id: clientId, q3_phase: "align" });
      toast.success("Chapter 3: Strengthen complete!");
      onPhaseComplete();
    } catch {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* nextPhase === null means this is the last phase — show celebration card */}
      {nextPhase === null ? (
        <div className="rounded-xl border border-emerald-300 bg-emerald-50 px-5 py-4 space-y-3">
          ...
          <p className="text-xs text-emerald-700">
            {allDone
              ? "All items complete — your protection architecture is in place."
          ...
          >
            {updateClient.isPending ? (
              ...
              <>
                <CheckCircle2 className="w-3.5 h-3.5" /> Complete Chapter 3
              </>
            )}
          </Button>
        </div>
